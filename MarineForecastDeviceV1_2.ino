// Marine Forecast Device v1.2 06/08/25
#include <WiFi.h>
#include <WebServer.h>
...
struct Config {
  ...
  char ssid[32]   = "";
  char pass[64]   = "";
  uint8_t brightness = 2;  // 0 = Low, 1 = Medium, 2 = High
} cfg;
...
void setup() {
  ...
  // Load config before using cfg.brightness
  if (!loadCfg() || cfg.ssid[0] == '\0') {
    cfg.brightness = 2;  // fallback default
    startAP();
    return;
  }
  if (cfg.brightness > 2) cfg.brightness = 2;

  // Now it's safe to set brightness
  const uint8_t BRIGHTNESS_LEVELS[] = { 64, 192, 255 };
  pinMode(TFT_BL, OUTPUT);
  analogWrite(TFT_BL, BRIGHTNESS_LEVELS[cfg.brightness]);

  // Initialize display and touch
  tft.init(); tft.setRotation(0);
  screenSprite.setColorDepth(16);
  screenSprite.createSprite(240, 160);
...
bool fetchData() {
  ...
  for (JsonObject p : per) {
    ...
    fp.timeISO     = p["startTime"].as<String>();
  ...
  if (omj != "") {
    ...
    if (!deserializeJson(jm, omj)) {
      ...
      for (int i = 0; i < ts.size() && marineCount < MAX_MARINE_POINTS; i++) {
        marineForecast[marineCount].timeStr    = ts[i].as<String>();
...
void handleRoot() {
  ...
  html += "Brightness: <select name='brightness'>";
  html += "<option value='0'" + String(cfg.brightness == 0 ? " selected" : "") + ">Low</option>";
  html += "<option value='1'" + String(cfg.brightness == 1 ? " selected" : "") + ">Medium</option>";
  html += "<option value='2'" + String(cfg.brightness == 2 ? " selected" : "") + ">High</option>";
  html += "</select><br>";
...
void handleSave() {
  //Serial.println("[handleSave] Received POST request");
  ...
  cfg.brightness = constrain(server.arg("brightness").toInt(), 0, 2);
  saveCfg();
  server.send(200, "text/html", "<h3>Saved. Rebooting…</h3>");
  delay(1000);
  WiFi.softAPdisconnect(true);  // Add this line
  delay(100);
  //Serial.println("[handleSave] Configuration saved, rebooting...");
  ESP.restart();
}
...
void loop() {
  ...
  static uint32_t lastGraphClear = 0;
  if (millis() - lastGraphClear > 3600000UL) {  // every 1 hour
    tft.fillRect(0, 0, 240, 140, TFT_BLACK);    // full tide area
    drawForecastScreen();                       // re-render cleanly
    lastGraphClear = millis();
  }
...
void handleTouch() {
  ...
  if (p.z > 50) {
    if (!touching) {
      tStart = millis(); touching = true; longDone = false;
